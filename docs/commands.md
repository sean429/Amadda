...
| 예시 입력 | 동작 |
|---|---|
| `넌 누구야` / `너 누구야` / `누구니` / `누구야` | Amadda가 자신을 소개 |
| `자기소개해줘` / `소개해줘` / `who are you` | 동일 |
...
| 웨이크워드 | 동작 |
|---|---|
| `아맞다` / `아맞다야` | 마이크 자동 활성화 → 이후 명령 음성 입력 |

> 웨이크워드는 silero-VAD + Whisper tiny 기반으로 오프라인 동작합니다.

---

## 참고

- 음성 입력 시 마이크 버튼을 누르거나 웨이크워드로 활성화 후 명령을 말합니다.
